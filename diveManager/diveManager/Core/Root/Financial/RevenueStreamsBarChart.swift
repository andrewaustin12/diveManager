    ...
    var revenueData: [RevenueStreamData] {
        ...
        for invoice in filteredInvoices {
            for item in invoice.items {
                if item.amount > 0 { // Ensure only positive values are aggregated
                    revenueTotals[item.category, default: 0] += item.amount
                }
            }
        }
        
        return revenueTotals.filter { $0.value > 0 } // Exclude zero values
                          .map { RevenueStreamData(type: $0.key, totalAmount: $0.value) }
    }
